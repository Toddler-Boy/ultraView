...
class GUI_LookAndFeel final : public juce::LookAndFeel_V4, public juce::MouseListener
{
public:
	GUI_LookAndFeel ();
	...
	// Point size = the em size (CSS-comparable); the shared framework code
	// sizes all its text this way. Resolve to a concrete height, point-only
	// fonts break juce::TextEditor
	[[ nodiscard ]] juce::Font fontPoints ( const float points, const int weight )
	{
		fontTableUsed[ weight / 100 ] = true;
		const auto	f = juce::Font ( fontTable[ weight / 100 ].withPointHeight ( points ) );
	...
	{
		const auto	f = juce::Font ( monoFont.withPointHeight ( points ) );
	...
	// Rounded outline with the inset-by-half and radius-shrink handled; width 0
	// hides the line (the shared components draw through this)
